...
<form method="GET" id="searchForm" class="space-y-4">
    ...
    <div class="flex flex-col mb-2">
        <div class="flex flex-lign items-center mb-2">
            <select id="selectDb" name="selectOption"
                    class="h-8 bg-gray-100 p-2 border border-[#b2a5ff] rounded-md mr-4 pt-1 pb-1">
                <option value="Topics" id="optionTopics" <?php if ($_GET['selectDb'] == 'Topics') echo 'selected'; ?>>Categories</option>
                <option value="Users" id="optionUsers" <?php if ($_GET['selectDb'] == 'Users') echo 'selected'; ?>>Utilisateurs</option>
                <option value="Posts" id="optionPosts" <?php if ($_GET['selectDb'] == 'Posts') echo 'selected'; ?>>Posts</option>
                <option value="Comments" id="otionComments" <?php if ($_GET['selectDb'] == 'Comments') echo 'selected'; ?>>Commentaires
            </select>

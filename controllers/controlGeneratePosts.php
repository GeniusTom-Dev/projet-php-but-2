    ...
    function getPostHTML(int $postID): string{
        ...
        <article class="postInterface w-full md:w-1/2 lg:w-1/3 xl:w-1/2 h-auto md:h-1/3 lg:h-auto xl:h-auto bg-gray-100 rounded-lg shadow-md p-6 mb-4">
            <header class="flex flex-lign items-center mb-2">
                ...
                <form action="/projet-php-but-2/View/affichagePostDetails.php" method="get"> <!-- Affichage page détail post -->
                    <input name="detailsPost" type="hidden" value="<?= $postID ?>">
                    <img src="/projet-php-but-2/html/images/fleches.svg" alt="growArrow" class="growArrow w-10 h-auto transition-transform duration-300 hover:scale-125 ml-auto" onclick="submit()">
                </form>
            </header>
                ...
                <div class="flex flex-lign items-center mb-2">
                    <form method="post">
                        <?php if (isset($_SESSION['suid']) && $this->dbLikes->doesUserHasLikedThisPost($_SESSION['suid'], $postID)){ ?>
                            ...
                            <img src="/projet-php-but-2/html/images/heart-solid.svg" alt="heart" class="heart w-8 h-auto transition-transform duration-300 hover:scale-125 mr-2" onclick="submit()">
                        <?php } else {?>
                            <input type="hidden" name="likePost" value="<?= $postID ?>">
                            <img src="/projet-php-but-2/html/images/heart-regular.svg" alt="heart" class="heart w-8 h-auto transition-transform duration-300 hover:scale-125 mr-2" <?php if (isset($_SESSION['suid'])) echo 'onclick="submit()"';?>>
                        <?php } ?>
                    </form>
                    ...
                    <form method="post">
                        <?php if (isset($_SESSION['suid']) && $this->dbFavorites->doesUserHaveFavoritedThisPost($_SESSION['suid'], $postID)){ ?>
                            ...
                            <img src="/projet-php-but-2/html/images/bookmark-solid.svg" alt="bookmark" class="bookmark w-6 h-auto transition-transform duration-300 hover:scale-125 mr-2" onclick="submit()">
                        <?php } else {?>
                            <input type="hidden" name="markPost" value="<?= $postID ?>">
                            <img src="/projet-php-but-2/html/images/bookmark-regular.svg" alt="bookmark" class="bookmark w-6 h-auto transition-transform duration-300 hover:scale-125 mr-2" <?php if (isset($_SESSION['suid'])) echo 'onclick="submit()"';?>>
                        <?php } ?>
                    </form>
                </div>
            </footer>
